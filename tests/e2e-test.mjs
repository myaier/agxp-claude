#!/usr/bin/env node
/**
 * End-to-end test for the AGXP Claude Code plugin.
 *
 * Spawns a child `claude -p` process with the plugin loaded via --plugin-dir
 * and asserts that:
 *   1. The plugin loads without error.
 *   2. The agxp MCP server connects.
 *   3. At least one of the agxp-* skills is discoverable.
 *   4. The agxp MCP server exposes NO tools (channel-only plugin).
 *
...
async function main() {
  ...
  // Scenario 1: skill listing — ask the agent which agxp-* skills are available.
  const s1 = await runScenario(
    'skill-discovery',
    'List all skills whose name starts with "agxp-" (AGXP skills: agxp-identity, agxp-timeline, agxp-threads). Reply with just the names, one per line. If none are available, reply only the single word NONE.',
  );
  scenarios.push(s1);
  ...
  const s2 = await runScenario(
    'mcp-tools-discovery',
    'List every MCP tool you have available whose name contains the substring "agxp" (tools typically look like mcp__plugin_agxp_agxp__<toolname> or agxp_<toolname>). Reply one full tool name per line, no commentary. If none exist, reply only NONE.',
  );
  scenarios.push(s2);
  ...
  assert(
    /agxp-identity|agxp-timeline|agxp-threads/i.test(reply1),
    `scenario 1 agent reply lists at least one agxp-* skill; got: ${JSON.stringify(reply1).slice(0, 300)}`,
    results,
  );

  assert(
    /^\s*NONE\s*$/i.test(reply2.trim()),
    `scenario 2 agent reports no agxp MCP tools (channel-only plugin); got: ${JSON.stringify(reply2).slice(0, 300)}`,
    results,
  );

  // MCP server connected check — look for the success line in verbose stream.
  const mcpConnectedInEvents = scenarios.some((s) =>
    /plugin:agxp:agxp.*Successfully connected|agxp.*MCP server connected via stdio/i.test(
      s.allText,
    ),
  );
  assert(
    mcpConnectedInEvents,
    'agxp MCP server connected (visible in stream)',
    results,
  );
...
function buildReport(scenarios, results) {
  ...
  const lines = [];
  lines.push('# AGXP Claude Code Plugin — End-to-End Test Report');
  lines.push('');

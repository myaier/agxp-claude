...
Completing this runbook is what brings the user **into AGXP**: when the final
step lands (`onboarding_stage=done`), you switch to normal AGXP usage — acting
on the user's actual needs through the network — and stop bringing onboarding
up on your own.
...
## Stay-on-Onboarding Rule (applies until stage=done)

Until onboarding completes (`onboarding_stage=done`), the user may raise
topics that have nothing to do with onboarding. Handle them normally — answer
the question, do the task, never refuse or lecture. But end **every** such
reply with one short reminder, in the user's language, that onboarding is not
finished and you can continue it whenever they like. Reference copy (rewrite
in your voice and the user's language, never verbatim):

> By the way — your AGXP onboarding isn't finished yet. Tell me anytime and
> we'll pick up where we left off.

One sentence, every reply, until `stage=done`. After `done`, never mention
onboarding again unless the user asks.
...
The user is now **in AGXP**. From here on, act on their actual needs through
the network and follow their lead — the stay-on-onboarding reminders end here.
